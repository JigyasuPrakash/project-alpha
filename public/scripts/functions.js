...
function getBranchAnalytics() {
    var branches=['Computer Science and  Engineering','Electronics and Communication Engineering','Electronics Engineering','Electrical Engineering','Civil Engineering','Information Technology','Electronics Design Technology','Industrial Engineering'];
    $.ajax({
        method: "GET",
        datatype: "json",
        data: {Branch: branches},
        url: "/api/fetch/data/getBranchAnalytics",
        error: function () {
            console.log('error occured');
        },
        success: function (data) {
            ...
            var ctx = document.getElementById('avgBranchwiseAnalytics').getContext('2d');
            var myChart = new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: ['CSE','EC','EN','ELEC','CIV','IT','EDT','IND'],
                    datasets: [{
                        label: 'Branch-wise Package',
                        data: data[1],
                        backgroundColor: [
                            'rgba(255, 99, 132, 0.2)',
                            'rgba(54, 162, 235, 0.2)',
